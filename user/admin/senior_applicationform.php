...
<?php
        ...
        $date = date('mdy');
        $date_as = date('m/d/Y');
        $query_am = mysqli_query($connection,"SELECT COUNT(*)AS amses FROM senior_applicant_information WHERE date_exam = '$date_as' AND time_exam = '9:00 AM'");
        $amses_result = mysqli_fetch_assoc($query_am);
        $amses = $amses_result["amses"];
        $query_pm = mysqli_query($connection,"SELECT COUNT(*)AS pmses FROM senior_applicant_information WHERE date_exam = '$date_as' AND time_exam = '1:30 PM'");
...
<?php
  ...
  if(isset($_POST["applicant_btn"])){
    ...
    $homeadd = mysqli_real_escape_string($connection,$_POST["home"]);
    ...
    if($time_exam == "9:00 AM"){
      ...
      if($fname && $mname && $lname && $date_exam && $time_exam && $age && $gender && $telno && $mobileno && $school && $schoolyr && $homeadd && $citizen && $emailadd && $firstchoice && $secondchoice && $thirdchoice){
        ...
        if($check > 0){
          ...
          mysqli_query($connection,"INSERT INTO senior_applicant_information(applicant_no,fname,mname,lname,age,gender,telno,mobileno,previous_school,homeadd,academicyear,citizenship,emailadd,firstchoice,secondchoice,thirdchoice,date_exam,time_exam) VALUES ('$generate_appno','$fname','$mname','$lname','$age','$gender','$telno','$mobileno','$school','$homeadd','$schoolyr','$citizen','$emailadd','$firstchoice','$secondchoice','$thirdchoice','$date_exam','$time_exam')");
  ...
  <body>
    ...
    <div class="main-content">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            <h1 class="h2">Senior High Application Form</h1>
      </div>
       <div class="application-holder">
        ...
        <table width="95%">
          ...
          <tr>
            ...
            <td>
              <label for="school" class="col-form-label">PREVIOUS SCHOOL:</label>
              <input type="text" name="school" id="school" class="form-control" required="">
            </td>
          </tr>
          <tr>
            <td>
              <label for="home" class="col-form-label">HOME ADDRESS:</label>
              <input type="text" name="home" id="home" class="form-control" required="">
            </td>
            <td>
              <label for="schoolyr" class="col-form-label">ACADEMIC YEAR:</label>
