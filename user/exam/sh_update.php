...
if(@$_GET['q']== 'quiz' && @$_GET['step']== 2) {
	...
	{
	$sn++;
	header("location:sh_account.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total")or die('Error152');
	}
	...
	header("location:sh_account.php?q=1");
	}
	else
	{
	header("location:sh_account.php?q=1");
...
$sun=$sun-$s;
$q=mysqli_query($connection,"UPDATE `rank` SET `score`=$sun ,time=NOW() WHERE email= '$email'")or die('Error174');
header("location:sh_account.php?q=quiz&step=2&eid=$eid&n=1&t=$t");
}
