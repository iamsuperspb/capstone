...
echo '<span class="pull-right top title1" ><span class="log1"><span class="glyphicon glyphicon-user" aria-hidden="true"></span>&nbsp;&nbsp;&nbsp;&nbsp;Hello,</span> <a href="cl_account.php?q=1" class="log log1">'.$name.'</a>&nbsp;|&nbsp;<a href="logout.php?q=cl_account.php" class="log"><span class="glyphicon glyphicon-log-out" aria-hidden="true"></span>&nbsp;Signout</button></a></span>';
}?>
...
<?php
  ...
  if(isset($_POST['finish_btn'])){
    ...
    $date = date('F d, Y');
  // GET COURSE
  $get_month = mysqli_query($connection,"SELECT MONTH(now()) as month FROM college_applicant_information");
  $get_month_result = mysqli_fetch_array($get_month);
  $month_result = $get_month_result["month"];
  $get_course = mysqli_query($connection,"SELECT * FROM college_applicant_information WHERE applicant_no = '$session_appno'");
  ...
  if($total_avg < 60 && $total_avg >=40){
     $insert_score = mysqli_query($connection,"INSERT INTO college_exam_score (applicant_no,name,email,score,status,first_course,second_course,third_course,date_exam,time_exam,month_report)VALUES('$session_appno','$std_name','$std_email','$total_avg','For Interview','$firstcourse','$secondcourse','$thirdcourse','$date_exam','$time_exam','$month_result')");
  }
  if($total_avg >= 60 && $total_avg <=100){
     $insert_score = mysqli_query($connection,"INSERT INTO college_exam_score (applicant_no,name,email,score,status,first_course,second_course,third_course,date_exam,time_exam,month_report)VALUES('$session_appno','$std_name','$std_email','$total_avg','Passed','$firstcourse','$secondcourse','$thirdcourse','$date_exam','$time_exam','$month_result')");
  }
  if($total_avg < 40 && $total_avg >=0){
     $insert_score = mysqli_query($connection,"INSERT INTO college_exam_score (applicant_no,name,email,score,status,first_course,second_course,third_course,date_exam,time_exam,month_report)VALUES('$session_appno','$std_name','$std_email','$total_avg','Failed','$firstcourse','$secondcourse','$thirdcourse','$date_exam','$time_exam','$month_result')");
  }
  }
?>
</div>
...
// VALIDATE IF IT THE USER SUBMIT THE Exam
 $validate_exam = mysqli_query($connection,"SELECT COUNT(*) as std_validate FROM college_exam_score WHERE email = '$email'");
...
if($rowcount == 0){
	...
	<td><b><a href="cl_account.php?q=quiz&step=2&eid='.$eid.'&n=1&t='.$total.'" class="pull-right btn sub1" style="margin:0px;background:#A62D38">&nbsp;<span class="title1"><b>Start</b></span></a></b></td></tr>';
}
else
{
...
echo '<tr style="color:#99cc32"><td>'.$c++.'</td><td>'.$title.'&nbsp;<span title="This quiz is already solve by you" class="glyphicon glyphicon-ok" aria-hidden="true"></span></td><td>'.$total.'</td>  
	<td><b><a href="cl_update.php?q=quizre&step=25&eid='.$eid.'&n=1&t='.$total.'"</a></b></td></td></tr>';
}
}
if($count_exam_std == 0){
...
$qid=$row['qid'];
if($image_qns != "" && $qns != ""){
echo "<b>Question &nbsp;".$sn."&nbsp;<br /><img src='../../plugin/exam_image/".$image_qns."' height='200px' width='200px'></b><br /><br />";
echo '<b>'.$qns.'</b></br></br>';
}
elseif($image_qns != ""){
...
elseif($qns != ""){
  echo '<b>Question &nbsp;'.$sn.'&nbsp;<br /><b>'.$qns.'</b></br></br>';
}
// if($image_qns != "" && $qns != ""){
...
echo '<form name=quiz action="cl_update.php?q=quiz&step=2&eid='.$eid.'&n='.$sn.'&t='.$total.'&qid='.$qid.'" method="POST"  class="form-horizontal">
<br />';
...
// echo'<table><button type="submit" class="btn btn-success" style="width:100px;"><span class="glyphicon glyphicon-chevron-right" aria-hidden="true"></span> Next</button></form></table>';
...
<center><h1 class="title" style="color:#660033">Finish</h1><center><br /><a href="cl_account.php?q=1"><button class ="btn btn-success"><span class = "glyphicon glyphicon-ok"></span> Finish</button></a>';
