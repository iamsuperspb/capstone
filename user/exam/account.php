...
<?php
  ...
  if($total_avg <= 74){
     $insert_score = mysqli_query($connection,"INSERT INTO college_exam_score (applicant_no,name,email,score,status,first_course,second_course,third_course,date_exam,time_exam)VALUES('$session_appno','$std_name','$std_email','$total_avg','For Interview','$firstcourse','$secondcourse','$thirdcourse','$date_exam','$time_exam')");
  }
  if($total_avg > 74 && $total_avg <=100){
     $insert_score = mysqli_query($connection,"INSERT INTO college_exam_score (applicant_no,name,email,score,status,first_course,second_course,third_course,date_exam,time_exam) VALUES('$session_appno','$std_name','$std_email','$total_avg','Passed','$date','$firstcourse','$secondcourse','$thirdcourse','$date_exam','$time_exam')");
  }
  }
?>
</div>
...
// COUNTING SUBJECT
  $query_count = mysqli_query($connection,"SELECT COUNT(*) AS quiz FROM quiz");
...
if($rowcount == 0){
	...
	<td><b><a href="account.php?q=quiz&step=2&eid='.$eid.'&n=1&t='.$total.'" class="pull-right btn sub1" style="margin:0px;background:#A62D38">&nbsp;<span class="title1"><b>Start</b></span></a></b></td></tr>';
}
else
{
...
echo '<tr style="color:#99cc32"><td>'.$c++.'</td><td>'.$title.'&nbsp;<span title="This quiz is already solve by you" class="glyphicon glyphicon-ok" aria-hidden="true"></span></td><td>'.$total.'</td>  
	<td><b><a href="update.php?q=quizre&step=25&eid='.$eid.'&n=1&t='.$total.'"</a></b></td></td></tr>';
}
}
if($count_exam_std == 0){
...
$q=mysqli_query($connection,"SELECT * FROM questions  WHERE eid='$eid' AND sn = '$current_number'" );
...
echo '<form name=quiz action="update.php?q=quiz&step=2&eid='.$eid.'&n='.$sn.'&t='.$total.'&qid='.$qid.'" method="POST"  class="form-horizontal">
<br />';
...
echo'<table><button type="button" onclick="history.back();" class="btn btn-warning" style="width:100px;"><span class="glyphicon glyphicon-chevron-left" aria-hidden="true"></span> Previous</button>&nbsp;<button type="submit" class="btn btn-success" style="width:100px;"><span class="glyphicon glyphicon-chevron-right" aria-hidden="true"></span> Next</button></form></table>
...
<center><h1 class="title" style="color:#660033">Finish</h1><center><br /><a href="account.php?q=1"><button class ="btn btn-success"><span class = "glyphicon glyphicon-ok"></span> Finish</button></a>';
