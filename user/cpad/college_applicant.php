...
<?php
  if(isset($_POST["edit_btn"])){
    ...
    mysqli_query($connection,"UPDATE college_applicant_information SET fname = '$fname', mname = '$mname', lname = '$lname', firstchoice = '$firstchoice', secondchoice = '$secondchoice', thirdchoice = '$thirdchoice' WHERE applicant_no = '$id'");
      ...
      <p>
       ...
       <!-- <form method="POST">
          ...
          if(isset($_POST["search_btn"])){
                ...
                $start = 0;
                $limit = 5;
                $total = mysqli_query($connection,"SELECT * FROM college_applicant_information WHERE fname LIKE '%".$search."%'");
              ...
              $fetch = mysqli_query($connection,"SELECT * FROM college_applicant_information WHERE fname LIKE '%".$search."%' LIMIT $start,$limit");
            }
            else if(isset($_POST["cl_search_btn"])){
              ...
              $fetch = mysqli_query($connection,"SELECT * FROM college_applicant_information WHERE date_exam ='$date_view' AND status = '$viewby' ORDER BY applicant_no ASC LIMIT $start,$limit");
                ...
                $start = 0;
                $limit = 5;
                $total = mysqli_query($connection,"SELECT * FROM `college_applicant_information` ORDER BY `college_applicant_information`.`date_exam` DESC");
                $total_rows = mysqli_num_rows($total);
                if(isset($_GET["id"])){
                  $id = $_GET["id"];
                  $start = ($id-1)*$limit;
                ...
                $page = ceil($total_rows/$limit);
              $fetch = mysqli_query($connection,"SELECT * FROM `college_applicant_information` ORDER BY `college_applicant_information`.`date_exam` DESC LIMIT $start,$limit");
              }
          ?>
